...
function TextEditor() {
  ...
  useEffect(() => {
    ...
    }// Fetch user project if logged in
  ...
  const saveProject = async () => {
    ...
    const response = await fetch(api_base_url + "/saveProject", {
      ...
      body: JSON.stringify({ htmlCode: html, cssCode: css, jsCode: js })
    });

    const data = await response.json();
    if (data.success) alert("Project saved successfully.");
  ...
  return (
    <>
      <div className="flex justify-center gap-3">
        <button
          onClick={runCode}
          className="bg-purple-700 hover:bg-purple-500 text-white font-medium sm:py-2 sm:px-5 px-2 py-1 rounded hover:cursor-pointer"
        >
          Run
        </button>
        <button onClick={saveProject} className="bg-white text-purple-700 hover:text-purple-500 font-medium sm:py-2 sm:px-5 px-2 py-1 rounded hover:cursor-pointer">Save</button>
      </div>
